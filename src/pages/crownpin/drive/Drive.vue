    ...
    <search-bar @find="find" @clear="clear" @searchIshidden=searchIshidden>
      ...
      <template slot="searchFilter">
          ...
          <el-row class="m-b-30">
            <el-col :span="6">
              <el-row type="flex" align="middle">
                ...
                <el-col :span="17">
                  <select v-model="formOption.advisor">
                   <!--  <option v-for="option in formOption.accoutOptions" v-bind:value="option.accountId">
                      {{ option.nickname }}
                    </option> -->
                     <option v-for="option in formOption.accoutOptions" v-bind:value="option">
                      {{ option.nickname }}
                    </option>
                  </select>
                </el-col>
            ...
            <el-col :span="6">
              <el-row type="flex" align="middle">
                ...
                <el-col :span="17">
                  <el-date-picker size="small" :clearable="false"  v-model="filter.send" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd">
                  </el-date-picker>
                </el-col>
              </el-row>
            </el-col>
            <el-col :span="6">
              <el-row type="flex" align="middle">
                ...
                <el-col :span="17">
                  <el-date-picker size="small" :clearable="false"  v-model="filter.drive" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd">
                  </el-date-picker>
                </el-col>
              </el-row>
            </el-col>
        ...
        <div class="searchClose" v-if="!searchIsOpen">
             <ul>
               <li v-if="formOption.advisor && formOption.advisor.nickname">{{formOption.advisor.nickname}}</li>
               <li v-if="filter.keyword !=undefined">{{filter.keyword}}</li>
               <li v-if="filter.send !=false">{{filter.send |normalTime}}</li>
               <li v-if="filter.drive !=false">{{filter.drive |normalTime}}</li>
               <li v-if="filter.type !=undefined">{{filter.type==1?"是":"否"}}</li>
               <li v-if="filter.over_time !=undefined">{{filter.over_time==1?"是":"否"}}</li>
             </ul>
        </div>
        <!--收起时显示的内容end-->
...
export default {
  ...
  data() {
    return {
      formOption: {accoutOptions:[],advisor:{},typeOptions:[],over_timeOptions:[]},
      filter: {
        page: 1,
        drive:[],
        send:[],
  ...
  methods: {
    ...
    find() {
      ...
      this.filter.advisor_id=this.formOption.advisor.accountId;
      this.searchTable()
    },
    //清除搜索条件
    clear() {
